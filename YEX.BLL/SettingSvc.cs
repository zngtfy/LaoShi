...
namespace YEX.BLL
{
    ...
    /// Setting service
    /// </summary>
    public class SettingSvc : GenericSvc<SettingRep, Setting>
    {
        #region -- Overrides --
        ...
        /// <returns>Return the result</returns>
        public override MultipleRsp Create(List<Setting> l)
        {
            var res = new MultipleRsp();
            var lErr = new List<SettingDto>();
            var lOki = new List<Setting>();
            ...
            foreach (var i in l)
            {
                var m = _rep.Read(i.Code);
                if (m != null)
                {
                    lErr.Add(new SettingDto(i));
                }
                else
                {
                    lOki.Add(i);
            ...
            if (lOki.Count > 0)
            {
                ...
                res.SetSuccess(SettingDto.ToDto(lOki), string.Empty);
            }
        ...
        /// <param name="l">List model</param>
        /// <returns>Return the result</returns>
        public override MultipleRsp Update(List<Setting> l)
        {
            var res = new MultipleRsp();
            var lErr = new List<SettingDto>();
            var lOki = new List<Setting>();
            ...
            foreach (var i in l)
            {
                var m = i.Id > 0 ? _rep.Read(i.Id) : _rep.Read(i.Code);
                if (m == null)
                {
                    lErr.Add(new SettingDto(i));
                }
