...
namespace YEX.BLL
{
    ...
    public class RoleSvc : GenericSvc<RoleRep, Role>
    {
        ...
        /// <returns>Return the result</returns>
        public override MultipleRsp Create(List<Role> l)
        {
            var res = new MultipleRsp();
            var lErr = new List<RoleDto>();
            var lOki = new List<Role>();
            ...
            if (lOki.Count > 0)
            {
                ...
                res.SetSuccess(RoleDto.ToDto(lOki), string.Empty);
            }
        ...
        public override SearchRsp Read(PagingReq req)
        {
            ...
            try
            {
                ...
                if (req.Filter != null)
                {
                    filter = JsonConvert.DeserializeObject<RoleFilter>(req.Filter.ToString());
                }
        ...
        /// <param name="l">List model</param>
        /// <returns>Return the result</returns>
        public override MultipleRsp Update(List<Role> l)
        {
            var res = new MultipleRsp();
            var lErr = new List<RoleDto>();
            var lOki = new List<Role>();
